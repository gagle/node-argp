...
Argp._validateTrailing = function (trailing){
	if (!trailing) return;
	...
	if (eq && (min || max)){
		throw new ArgpError ("Cannot configure \"trailing.eq\" along with " +
				"\"trailing.min\" or \"trailing.max\"");
	}
	
	if (eq && trailing.eq < 1){
		throw new ArgpError ("\"trailing.eq\" must be greater than 1");
	}
	if (min && trailing.min < 0){
		throw new ArgpError ("\"trailing.min\" must be greater than 1");
	}
	if (max && trailing.max < 1){
		throw new ArgpError ("\"trailing.max\" must be greater than 1");
	}
	...
	if (min && max){
		if (trailing.min === trailing.max){
			...
			throw new ArgpError ("\"trailing.max\" must be greater or equal than " +
					"\"trailing.min\"");
		}
	}
	if (!min){
...
Argp.prototype._printHelp = function (){
	...
	if (this._usages.length){
		...
		h = wrap ("Usage: " + script + (this._command ? " " + this._name : "") + h,
				this._columns, prefix);
	}
	...
	this._lines.forEach (function (o){
		...
		h += me._columnize (line + "  ", o.description);
...
Argp.prototype._printUsage = function (){
	...
	if (this._usages.length){
		...
		usage = wrap ("Usage: " + script + (this._command ? " " + this._name : "") +
				usage, this._columns, prefix);
	}
...
Argp.prototype._default = function (){
	var me = this;
	this._lines.forEach (function (o){
		if (o.paragraph || o.line || o.group || o.columns || o.arg) return;
...
Argp.prototype.commands = function (name){
