				...
				if (o.flag){
					...
					if (!i){
						...
						if (ii > 1){
							...
							//non-flag option, the next undefined characters to the first
							//defined option are the value of this option, eg: -Vabc, where
							//"V" is a defined non-flag option and "abc" is the value
...
Argp.prototype.argv = function (input){
	...
	if (input){
		...
		if (end) end (o, instance._printHelpBound, instance._printUsageBound,
				instance._printVersionBound, instance._failBound);
		return o;
	}
	...
	if (end){
		//In order to avoid creating unnecessary messages, create only the messages
		...
		if (length > 1){
			helpString = instance._createHelp ();
			arr.push (function (){
		...
		if (length > 2){
			usageString = instance._createUsage ();
			arr.push (function (){
		...
		if (length > 3){
			versionString = instance._createVersion ();
			arr.push (function (){
		...
		if (length > 4){
			fail = instance._createFail ();
			arr.push (function (str, code){
