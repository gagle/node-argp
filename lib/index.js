...
var Argp = function (){
	...
	//The parsed options are stored into an array because if inOrder is false
	//the arguments are evaluated after the options
	this._argv = [];
...
Argp.prototype._default = function (){
	var me = this;
	this._optionsArray.forEach (function (opt){
		if (opt.text) return;
		me._obj[opt.long || opt.short] = opt.flag ? !!opt.negate : opt.value;
	});
	for (var p in this._arguments){
		this._obj[p] = this._arguments[p];
	}
};

Argp.prototype._build = function (){
...
Object.defineProperty (Argp.prototype, "argv", {
	get: function (){
		...
		return this.configure ({ undefinedOptions: false }).parse ();
	}
});
...
Argp.prototype.parse = function (){
	...
	//Build the final object with the configuration and the input options
	this._build ();
